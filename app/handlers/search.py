...
async def _send_preview_card(bot: MovieBot, message: Message, query: str, items: list[dict], active_type: str) -> None:
    ...
    if not active_items:
        ...
        return
    caption = build_search_hub_caption(query, movie_count=len(movies), series_count=len(series), active_type=active_type)
    sent = await message.reply_text(caption, reply_markup=search_results_keyboard(bot.settings, active_items, movie_count=len(movies), series_count=len(series), active_type=active_type, page=0, page_size=bot.settings.result_page_size))
    bot.search_sessions[(sent.chat.id, sent.id)] = {
        ...
        "movies": [str(item["_id"]) for item in movies],
        "series": [str(item["_id"]) for item in series],
    }
...
async def _run_search(bot: MovieBot, message: Message, query: str, media_type: Optional[str]) -> None:
    ...
    if not query or len(query) < 2:
        return
    items = await bot.db_service.search_titles(query=query, media_type=media_type, limit=max(bot.settings.max_search_results, 50))
    if not items:
        await message.reply_text("Database එකේ matching result එකක් නැහැ.")
        return
