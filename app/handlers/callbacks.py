...
def _stage_photo(bot: MovieBot, title: dict | None = None, *, stage: str = "search", final: bool = False) -> str | None:
    if title and title.get("poster_url"):
...
async def _ensure_title_poster(bot: MovieBot, title: dict | None) -> dict | None:
    if not title:
        return title
    ...
    try:
        ...
        logging.warning("Poster refresh lookup failed for %s: %s", title.get("title"), exc)
        return title
    ...
    try:
        if poster_url:
            try:
                ...
                logging.warning("Inbox intro photo failed for %s: %s", title.get("title"), exc)
        ...
        try:
            ...
            logging.warning("copy_message failed for %s: %s", title.get("title"), exc)
