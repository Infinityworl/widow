...
def _legacy_payload(data: str):
    parts = (data or "").split("|")
    if not parts:
        return None
    action = parts[0]
    payload = type("LegacyPayload", (), {})()
    payload.action = action
    payload.title_id = parts[1] if len(parts) > 1 else ""
    ...
    try:
        if action == "st" and len(parts) > 1:
            payload.media_type = parts[1]
            return payload
        if action == "pick" and len(parts) > 1:
            return payload
        if action == "mq" and len(parts) > 2:
            payload.quality = parts[2]
            return payload
        if action == "mv" and len(parts) > 2:
            payload.media_file_id = parts[2]
            return payload
        if action == "dl" and len(parts) > 2:
            payload.media_file_id = parts[2]
            return payload
        if action == "ss" and len(parts) > 2:
            payload.season = int(parts[2])
            payload.page = int(parts[3]) if len(parts) > 3 else 0
            return payload
        if action == "sp" and len(parts) > 3:
            payload.season = int(parts[2]); payload.page = int(parts[3]); return payload
        if action == "se" and len(parts) > 3:
            payload.season = int(parts[2]); payload.episode = int(parts[3]); return payload
        if action == "eq" and len(parts) > 4:
            payload.season = int(parts[2]); payload.episode = int(parts[3]); payload.quality = parts[4]; return payload
        if action == "ev" and len(parts) > 2:
            payload.media_file_id = parts[2]; return payload
        # legacy stage-flow buttons
        if action == "sq" and len(parts) > 3:
            payload.season = int(parts[2]); payload.quality = parts[3]; return payload
        if action in {"sc", "pg"} and len(parts) > 4:
            payload.season = int(parts[2]); payload.quality = parts[3]; payload.codec = parts[4]; payload.page = int(parts[5]) if len(parts) > 5 else 0; return payload
        if action == "ep" and len(parts) > 2:
            payload.media_file_id = parts[2]; return payload
    except Exception:
        return None
    return None
...
def _series_variant_caption(bot: MovieBot, title: dict, *, season: int, episode: int, quality: str) -> str:
    ...
    meta_parts.append(media_type_label(title.get("media_type", "series")))
    ...
    if overview:
        lines.extend(["", overview[:400] + ("..." if len(overview) > 400 else "")])
    lines.extend(["", "🎚 පහළ codec + size button එකක් තෝරන්න.", "🔒 මේ buttons වැඩ කරන්නේ search කරපු user ට විතරයි."])
    return "\n".join(lines)
...
async def _show_series_seasons(bot: MovieBot, query: CallbackQuery, title: dict) -> None:
    ...
    await _render_card(
        query,
        poster_url=_stage_photo(bot, title, stage="season"),
        caption=build_stage_caption(bot.settings, title, stage="season"),
        reply_markup=seasons_keyboard(bot.settings, str(title["_id"]), seasons),
    )
...
async def _show_series_episode_page(bot: MovieBot, query: CallbackQuery, title: dict, *, season: int, page: int = 0) -> None:
    ...
    await _render_card(
        query,
        poster_url=_stage_photo(bot, title, stage="episode"),
        caption=build_stage_caption(bot.settings, title, stage="episode", season=season),
        reply_markup=season_episodes_keyboard(bot.settings, str(title["_id"]), season, episodes, page, page_size, total_count),
    )
...
async def _show_download_ready(bot: MovieBot, query: CallbackQuery, title: dict, item: dict) -> None:
    ...
    await _render_card(
        ...
        poster_url=_stage_photo(bot, title, final=True),
        caption=caption,
        reply_markup=download_keyboard(bot.settings, str(title["_id"]), str(item["_id"])),
    )
...
async def _admin_show_title(bot: MovieBot, query: CallbackQuery, title_id: str) -> None:
    ...
    await _render_card(
        ...
        poster_url=title.get("poster_url"),
        caption=caption,
        reply_markup=admin_title_keyboard(bot.settings, title_id, title.get("media_type", "movie")),
    )


@MovieBot.on_callback_query(filters.regex(r"^(st|pick|mq|mv|dl|ss|sp|se|eq|ev|sq|sc|ep|pg)\|"))
async def user_callback_router(bot: MovieBot, query: CallbackQuery) -> None:
    ...
    try:
        ...
        payload = _legacy_payload(query.data or "")
        if payload is None:
    ...
    if payload.action == "pick":
        ...
        if title.get("media_type") == "series":
            ...
            await _show_movie_selector(bot, query, title)
        return
    ...
    if payload.action == "ss":
        await query.answer()
        await _show_series_episode_page(bot, query, title, season=payload.season or 1, page=0)
        return
    ...
    if payload.action in {"sc", "pg"}:
        await query.answer()
        await _show_series_episode_page(bot, query, title, season=payload.season or 1, page=payload.page or 0)
        return
    ...
    if payload.action == "eq":
        await query.answer()
        await _show_series_variants(
            bot,
            query,
            title,
            season=payload.season or 1,
            episode=payload.episode or 1,
...
async def admin_callback_router(bot: MovieBot, query: CallbackQuery) -> None:
    ...
    action_aliases = {
        ...
        "fd": "delfile",
        "tn": "edit_title_name",
        "tp": "edit_title_poster",
        "fq": "edit_file_quality",
        "fc": "edit_file_codec",
    }
    ...
    if action == "poster":
        ...
        await _render_card(
            query,
            poster_url=title.get("poster_url"),
            caption=build_admin_title_caption(title) + "\n\n🖼 Poster tools වලින් option එකක් තෝරන්න.",
            reply_markup=admin_poster_keyboard(bot.settings, title_id),
        )
        return
    ...
    if action == "posterauto":
        ...
        if not title:
            await query.answer("Title not found", show_alert=True)
            return
        metadata = await bot.metadata_service.search(
            title.get("title", ""),
            title.get("media_type", "movie"),
            title.get("year"),
        )
        updates = {
    ...
    if action == "variants":
        ...
        await query.message.reply_text(
            "Edit කරන්න variant එකක් තෝරන්න. Quality / Codec වෙනස් කරාම DB buttons auto update වෙයි.",
            reply_markup=admin_variant_picker(bot.settings, title_id, variants),
        )
        return
    ...
    if action == "delask":
        ...
        await query.message.reply_text(
            ...
            reply_markup=admin_confirm_delete_title_keyboard(bot.settings, title_id),
        )
        return
    ...
    if action == "deltitle":
        ...
        await query.message.reply_text(
            "✅ Title + related files DB එකෙන් delete කළා.",
    ...
    if action == "delfile":
        if len(parts) > 3:
            ...
            media_file_id = parts[2]
            media = await bot.db_service.get_media_file(media_file_id)
            title_id = str(media.get("title_id") or "") if media else ""
    ...
    if action == "edit_title_name":
        bot.admin_states[user_id] = {"mode": "await_edit_title_name", "title_id": parts[2]}
        await query.answer()
        await query.message.reply_text("Reply with: New title name")
        return

    if action == "edit_title_poster":
        bot.admin_states[user_id] = {"mode": "await_edit_title_poster", "title_id": parts[2]}
        await query.answer()
        await query.message.reply_text("Reply with: Poster URL / auto / remove")
        return

    if action == "edit_file_quality":
        media_file_id = parts[2]
        media = await bot.db_service.get_media_file(media_file_id)
        if not media:
            await query.answer("Variant not found", show_alert=True)
            return
        bot.admin_states[user_id] = {"mode": "await_edit_file_quality", "title_id": str(media.get("title_id") or ""), "media_file_id": media_file_id}
        await query.answer()
        await query.message.reply_text("Reply with: New quality (480p / 720p / 1080p / 2160p / 4K)")
        return

    if action == "edit_file_codec":
        media_file_id = parts[2]
        media = await bot.db_service.get_media_file(media_file_id)
        if not media:
            await query.answer("Variant not found", show_alert=True)
            return
        bot.admin_states[user_id] = {"mode": "await_edit_file_codec", "title_id": str(media.get("title_id") or ""), "media_file_id": media_file_id}
        await query.answer()
        await query.message.reply_text("Reply with: New codec (x264 / x265 / H264 / H265 / HEVC / AV1)")
        return
    ...
    if action == "edit":
        ...
        if target == "title":
            bot.admin_states[user_id] = {
                "mode": f"await_edit_title_{field}",
                "title_id": title_id,
            }
