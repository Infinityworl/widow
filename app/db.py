...
class Database:
    ...
    async def get_available_codecs(self, title_id: str, quality: str, season: Optional[int] = None) -> List[str]:
        match: Dict[str, Any] = {"title_id": ObjectId(title_id), "quality": quality}
        if season is not None:
            match["season"] = season
    ...
    async def get_episode_qualities(self, title_id: str, season: int, episode: int) -> List[str]:
        pipeline = [
            {"$match": {"title_id": ObjectId(title_id), "season": season, "episode": episode}},
            {"$group": {"_id": "$quality"}},
            {"$sort": {"_id": 1}},
        ]
    ...
    async def list_episode_variants(self, title_id: str, season: int, episode: int, quality: str, limit: int = 20) -> List[Dict[str, Any]]:
    ...
    async def get_movie_variant(self, title_id: str, quality: str, codec: str) -> Optional[Dict[str, Any]]:
        return await self.media_files.find_one(
            {"title_id": ObjectId(title_id), "quality": quality, "codec": codec},
