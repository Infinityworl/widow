...
def decode(data: str) -> CallbackPayload:
    ...
    if action == "mqp":
        return CallbackPayload(action=action, title_id=parts[1], page=int(parts[2]) if len(parts) > 2 else 0)
    if action == "mq":
        return CallbackPayload(action=action, title_id=parts[1], quality=parts[2], page=int(parts[3]) if len(parts) > 3 else 0)
    if action == "mvp":
        return CallbackPayload(action=action, title_id=parts[1], quality=parts[2], page=int(parts[3]) if len(parts) > 3 else 0)
    if action == "mv":
