...
NOISE_PATTERNS = [
    ...
    r"\b(imax|psa|yts|ytsmx|mx|rarbg|galaxyrg|evo|ettv|tgx|pahe(?:[ ._-]?in)?|amzn|nf|dsnp|hmax|proper|repack|extended|uncut|multi|dubbed|dual[ ._-]?audio)\b",
    r"\b(mkv|mp4|avi)\b",
]


def _format_size(num_bytes: int | None) -> Optional[str]:
...
def _clean_title_candidate(candidate: str) -> str:
    ...
    candidate = re.sub(r"\b\d{3,4}mb\b", " ", candidate, flags=re.I)
...
def parse_channel_message(message: Message) -> Optional[ParsedMedia]:
    ...
    size_label = parsed_lines.get("size_label") or _format_size(getattr(media, "file_size", None))
