...
class CatalogService:
    ...
    async def ingest_parsed_media(self, parsed: ParsedMedia, force_media_type: Optional[str] = None) -> Dict[str, Any]:
        ...
        canonical_title = metadata.get("title") if metadata else parsed.title
        title_doc = {
            "media_type": media_type,
            "title": canonical_title,
            "normalized_title": parsed.normalized_title,
            "aliases": list({parsed.title, canonical_title, *(filter(None, [metadata.get("original_title") if metadata else None]))}),
