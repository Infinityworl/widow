...
def search_results_keyboard(
    ...
    *,
    ...
    rows: List[List[InlineKeyboardButton]] = [[
        *_category_row(settings, movie_count=movie_count, series_count=series_count)
    ]]
    ...
    if page > 0:
        nav.append(
            InlineKeyboardButton(
                ui_text(settings, "prev_button_text", "⬅️ Prev"),
                callback_data=encode(["st", active_type, page - 1]),
            )
        )
    if end < total_count:
        nav.append(
            InlineKeyboardButton(
                ui_text(settings, "next_button_text", "Next ➡️"),
                callback_data=encode(["st", active_type, page + 1]),
            )
        )
    if nav:
...
def admin_variant_edit_keyboard(settings, title_id: str, media_file_id: str) -> InlineKeyboardMarkup:
    short_title_id = title_id[-8:]
    short_media_id = media_file_id[-8:]
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton(ui_text(settings, "admin_change_quality_button_text", "📦 Change Quality"), callback_data=f"adm|efq|{short_title_id}|{short_media_id}")],
            [InlineKeyboardButton(ui_text(settings, "admin_change_codec_button_text", "⚙️ Change Codec"), callback_data=f"adm|efc|{short_title_id}|{short_media_id}")],
            [InlineKeyboardButton(ui_text(settings, "admin_delete_file_button_text", "🗑 Delete This File"), callback_data=f"adm|df|{short_title_id}|{short_media_id}")],
